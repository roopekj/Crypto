...
def pad(string, length):    # Pads the string-variable with pkcs7 up to the length-variable or a multiple of it
    ...
    if str_len > length:
        ...
        num = length - str_len
        return string + (bytes([num]) * num)
def pkcs7_strip(s):     # Check for whether or not the s-parameter in padded with pkcs7 (raises an exception if not)
    byte = s[-1]
...
# NOTE: The CBC-mode encryption/decryption-functions could be made significantly simpler by using the pycryptodome's built-in cbc-mode, but that would be against the rules of some of the exercises they are used in.
...
def calculate_hamming_distance(first, second):
    ...
    for i in range(min(len(a), len(b))):
        if a[i] != b[i]:
            count = count + 1
    return count
def freq(string):   # Frequency of characters that suggest the string-parameter is written in english
    count = 0
    letters = list(
    ...
    print('The frequency in \t\'' + str(string) + '\'\t was ' + str(count / len(string)))
    return count / len(
        string)  # We divide the found characters by the length of the input so that longer inputs don't have an advantage in the evaluation
def create_kv(s):
    if '&' in s:
        parts = s.split('&')
    else:
...
def break_oracle(function):     # Finds the secret by which the input-oracle is extending its parameter
    #   First we need to isolate the oracle's potential prefix. We don't really need to know it's length or contents, just how many bytes of nonsense we need to feed the oracle until the prefix stops being a factor in the oracle's outputs
    ...
    prefix = b''    # This is the nonsense we need to start our input with
    for amount_of_nonsense in range(1, 17):
        output = function(b'\x21' * amount_of_nonsense)     # The oracle-function's extension shouldn't begin with the \x21-character, otherwise this process may fail
        first_16 = output[multiplier_default * AES.block_size:(multiplier_default + 1) * AES.block_size]
        if first_16 == previous_16:     # If the last 16 bytes of output don't change, we know the amount of nonsense required to evade the prepend of the oracle
    ...
    print('\nProcess took: ' + str(datetime.datetime.now() - time_1) + ' hours')
